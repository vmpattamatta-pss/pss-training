...
public class EmployeeDetailsDAO {
	...
	public EmployeeDetailsBean getById(Long id) {
		EmployeeDetailsBean ret = new EmployeeDetailsBean();
		try {
			if (conn == null || conn.isClosed())
				init();
			PreparedStatement stmt = conn.prepareStatement("select * from `java_training`.`employee_details`");
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				ret.setAddrLat(new BigDecimal(rs.getDouble("emp_addr_lat")));
				ret.setAddrLong(new BigDecimal(rs.getDouble("emp_addr_long")));
				ret.setAddrStr(rs.getString("emp_addr_str"));
				ret.setDesc(rs.getString("emp_desc"));
				ret.setDob(rs.getDate("emp_dob"));
				ret.setfName(rs.getString("emp_f_name"));
				ret.setGender(rs.getString("emp_gender"));
				ret.setId(rs.getLong("emp_id"));
				ret.setlName(rs.getString("emp_l_name"));
				ret.setProfileCvPath(rs.getString("emp_profile_cv_path"));
				ret.setProfilePicPath(rs.getString("emp_profile_pic_path"));
				ret.setSal(new BigDecimal(rs.getDouble("emp_sal")));
			}
			conn.close();
		} catch (Exception e) {
			e.printStackTrace();
